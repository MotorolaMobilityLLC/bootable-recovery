...
static const char* folder_to_pass[]={
                             "mcRegistry"
                     };
...
static bool dir_check( char const*dir)
{
    ...
    while ((dp = readdir(d_fd)) != NULL) {
        ...
        if (dp->d_type == DT_DIR){
             ...
             for(; idy < sizeof(folder_to_pass)/sizeof(char*); idy++){
               if(strstr(newdir, folder_to_pass[idy]) != NULL){
                               printf("Dir_check---found a folder to pass:  %s\n",newdir);
                               find_pass=1;
                               break;
               }
             }
                        ...
                        if(p_number==number)
                            {
                                ...
                                for(; idx < sizeof(folder_to_pass)/sizeof(char*); idx++)
                                {
                                    if(strstr(p_cmp_name, folder_to_pass[idx]) != NULL)
                                    {
                                        printf("list lost file---found a folder to pass:  %s\n",p_cmp_name);
                                        fclose(fp_info);
                                        return 0;
                                    }
                                }
